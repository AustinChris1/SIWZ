...
This is what dropping SIWZ into a new app looks like. ~150 lines of code,
of which **3 files** are the actual SIWZ wiring:

| File | Purpose | Lines |
|---|---|---|
| [`src/lib/auth.ts`](src/lib/auth.ts) | Hooks `SiwzProvider` into NextAuth | ~40 |
| [`src/app/api/siwz/nonce/route.ts`](src/app/api/siwz/nonce/route.ts) | Server-side nonce endpoint | ~10 |
| [`src/app/SignInClient.tsx`](src/app/SignInClient.tsx) | Drops `<SignInWithZcash />` in | ~30 |
...
- No memo-challenge UI yet (paste-flow only). The same `MemoSignIn` component used in ZBooks can be dropped in as a 1-line addition.

## What it proves

If you can do this in 150 lines on a Saturday, "Sign in with Zcash" is real infrastructure — same way SIWE became infrastructure. The packages, not the apps, are the contribution.
